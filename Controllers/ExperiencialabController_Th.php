...
class ExperiencialaboralController
{
    ...
    public function setExperiencialaboral($Empresa, $Cargo, $Fec_ini, $Fec_fin, $Emp_actual, $Horario, $Id_informacion_personal_exp_fk = 6) // Valor predeterminado
    {
        if (empty($Empresa) || empty($Cargo) || empty($Fec_ini) || empty($Fec_fin) || empty($Horario)) {
            echo '
            <script>alert("Completa todos los campos para poder registrar la experiencia laboral");
            window.location = "../html/VExp_lab1_Th.php";
            </script>
            ';
            exit;
        } else {
    ...
    public function deleteExperiencialaboral($Id_exp_laboral)
    {
        if (empty($Id_exp_laboral)) {
            ...
            window.location = "../html/VExp_lab_Th.php";
            </script>
            ';
            exit;
        } else {
    ...
    {
        if (empty($Id_exp_laboral) || empty($Empresa) || empty($Cargo) || empty($Fec_ini) || empty($Fec_fin) || empty($Horario) || empty($Id_informacion_personal_exp_fk)) {
            echo '
