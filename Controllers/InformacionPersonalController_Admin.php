...
class InformacionPersonalController  
{
    ...
    public function setInformacionPersonal($Num_documento, $Tipo_identificacion, $Fec_nacimiento, $Nombre, $Apellido, $Correo, $Telefono, $Direccion, $Fec_expedicion, $Ciudad, $Estrato, $Genero, $Nivel_educativo, $Foto, $Id_usuario_persona_fk = 1, $Id_post_pasantias_fk = 1)
    {
        ...
        if (empty($Num_documento) || empty($Tipo_identificacion) || empty($Fec_nacimiento) || empty($Nombre) || empty($Apellido) || empty($Correo) || empty($Telefono) || empty($Direccion) || empty($Fec_expedicion) || empty($Ciudad) || empty($Estrato) || empty($Genero) || empty($Nivel_educativo) || empty($Foto)){
            ...
            window.location = "../html/VInfo_personal1_Admin.php";
            </script>
            ';
            exit;
        }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['setInformacionPersonal'])) {
        ...
        $Id_post_pasantias_fk = !empty($_POST['Id_post_pasantias_fk']) ? $_POST['Id_post_pasantias_fk'] : 1; // Valor predeterminado si no se proporciona
