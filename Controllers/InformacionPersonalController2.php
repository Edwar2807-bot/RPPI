...
class InformacionPersonalController  
{
    ...
    {
        // Validar campos vacíos (incluyendo Foto)
        if (empty($Num_documento) || empty($Tipo_identificacion) || empty($Fec_nacimiento) || empty($Nombre) || empty($Apellido) || empty($Correo) || empty($Telefono) || empty($Direccion) || empty($Fec_expedicion) || empty($Ciudad) || empty($Estrato) || empty($Genero) || empty($Nivel_educativo) || empty($Foto)) {
            echo '
            <script>alert("Completa todos los campos, incluyendo la foto, para registrar la información personal");
            </script>
            ';
            exit;
        }
        ...
        if ($this->model->getByNumDocumento($Num_documento)) {
            ...
            window.location = "../html/VInfo_personal2.1.php";
            </script>
            ';
            exit;
        }
        ...
        echo '
            <script>alert("Información personal registrada");
            window.location = "../html/VInfo_personal2.php";
            </script>
        ';
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['setInformacionPersonal'])) {
        $Id_usuario_persona_fk = !empty($_POST['Id_usuario_persona_fk']) ? $_POST['Id_usuario_persona_fk'] : 1;
        $Id_post_pasantias_fk = !empty($_POST['Id_post_pasantias_fk']) ? $_POST['Id_post_pasantias_fk'] : 8;

        // Verifica si se ha enviado la foto
        if (!isset($_FILES['Foto']) || $_FILES['Foto']['error'] != UPLOAD_ERR_OK) {
            echo '<script>alert("Debes subir una foto.");</script>';
            exit;
        }

        $Foto = $_FILES['Foto']['name'];
        $InformacionPersonalController->setInformacionPersonal($_POST['Num_documento'], $_POST['Tipo_identificacion'], $_POST['Fec_nacimiento'], $_POST['Nombre'], $_POST['Apellido'], $_POST['Correo'], $_POST['Telefono'], $_POST['Direccion'], $_POST['Fec_expedicion'], $_POST['Ciudad'], $_POST['Estrato'], $_POST['Genero'], $_POST['Nivel_educativo'], $Foto, $Id_usuario_persona_fk, $Id_post_pasantias_fk);
        exit;
    }
